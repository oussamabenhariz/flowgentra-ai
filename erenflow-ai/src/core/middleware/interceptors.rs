...
//!
//! ```ignore
//! use erenflow_ai::core::middleware::{Middleware, ExecutionContext};
//! use std::sync::Arc;
//!
...
impl MiddlewarePipeline {
        ...
        for mw in &self.middleware {
            match mw.before_node(ctx).await {
                ...
                MiddlewareResult::Abort(reason) => {
                    return Err(ErenFlowError::ExecutionError(format!(
    ...
    pub async fn execute_on_complete(&self, final_state: &State) {
        for mw in &self.middleware {
...
pub struct MetricsMiddleware {
    metrics: std::sync::Arc<std::sync::Mutex<ExecutionMetrics>>,
}
...
impl MetricsMiddleware {
    ...
    pub fn new() -> Self {
        Self {
            metrics: std::sync::Arc::new(std::sync::Mutex::new(ExecutionMetrics::default())),
        }
    }
...
mod tests {
    ...
    async fn logging_middleware() {
        ...
        let mut ctx = ExecutionContext::new("test", State::new());
    ...
    async fn metrics_collection() {
        ...
        let mut ctx = ExecutionContext::new("node1", State::new());
        let _ = mw.after_node(&mut ctx).await;
    ...
    async fn validation_middleware_fails_missing_field() {
        let mw = ValidationMiddleware::new().require_field("required_field");
        let ctx = ExecutionContext::new("test", State::new());
        let mut ctx = ctx;
