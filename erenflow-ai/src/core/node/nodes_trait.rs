...
//!
//! ```ignore
//! use erenflow_ai::core::nodes::{PluggableNode, LLMNode};
...
//!
//! let result = node.run(State::new()).await?;
//! ```
...
///
/// # Type Parameter
/// - `S` - The state type (typically `State`)
#[async_trait]
pub trait PluggableNode: Send + Sync {
    ...
    fn validate_input(&self, state: &State) -> Result<()> {
        let _ = state;
        Ok(())
    }
...
impl Clone for Box<dyn PluggableNode> {
    fn clone(&self) -> Self {
        self.clone_box()
    }
}
