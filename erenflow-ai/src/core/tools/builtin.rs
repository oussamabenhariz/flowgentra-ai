...
impl CalculatorTool {
    ...
    fn evaluate(&self, operation: &str, a: f64, b: f64) -> Result<f64> {
        match operation {
            ...
            "divide" => {
                if b == 0.0 {
                    Err(ErenFlowError::ToolError("Division by zero".to_string()))
                } else {
                    Ok(a / b)
                }
            }
            ...
            "modulo" => {
                if b == 0.0 {
                    Err(ErenFlowError::ToolError("Modulo by zero".to_string()))
                } else {
                    Ok(a % b)
                }
            }
            _ => Err(ErenFlowError::ToolError(format!(
                "Unknown operation: {}",
                operation
            ))),
        }
...
impl Tool for CalculatorTool {
    async fn call(&self, input: Value) -> Result<Value> {
        let operation = input
            .get("operation")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ErenFlowError::ToolError("Missing 'operation' field".to_string()))?;

        let a = input
            .get("a")
            .and_then(|v| v.as_f64())
            .ok_or_else(|| ErenFlowError::ToolError("Missing or invalid 'a' field".to_string()))?;

        let b = input
            .get("b")
            .and_then(|v| v.as_f64())
            .ok_or_else(|| ErenFlowError::ToolError("Missing or invalid 'b' field".to_string()))?;
...
impl Tool for SearchTool {
    async fn call(&self, input: Value) -> Result<Value> {
        let query = input
            .get("query")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ErenFlowError::ToolError("Missing 'query' field".to_string()))?;
...
impl Tool for WebRequestTool {
    async fn call(&self, input: Value) -> Result<Value> {
        let url = input
            .get("url")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ErenFlowError::ToolError("Missing 'url' field".to_string()))?;
...
impl FilesTool {
    ...
    async fn read_file(&self, path: &str) -> Result<String> {
        fs::read_to_string(path)
            .map_err(|e| ErenFlowError::ToolError(format!("Failed to read file: {}", e)))
    }

    async fn write_file(&self, path: &str, content: &str) -> Result<()> {
        fs::write(path, content)
            .map_err(|e| ErenFlowError::ToolError(format!("Failed to write file: {}", e)))
    }

    async fn list_files(&self, path: &str) -> Result<Vec<String>> {
        let entries = fs::read_dir(path)
            .map_err(|e| ErenFlowError::ToolError(format!("Failed to list files: {}", e)))?;
        ...
        match operation {
            "read" => {
                let path = input
                    .get("path")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| ErenFlowError::ToolError("Missing 'path' field".to_string()))?;
            ...
            "write" => {
                ...
                let content = input
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        ErenFlowError::ToolError("Missing 'content' field".to_string())
                    })?;
