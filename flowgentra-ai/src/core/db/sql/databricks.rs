...
impl DatabricksDatabase {
    ...
    fn statements_url(&self) -> String {
        format!("{}/api/2.0/sql/statements", self.config.host.trim_end_matches('/'))
    }
        ...
        for _ in 0..60 {
            ...
            match state {
                ...
                "FAILED" | "CANCELED" | "CLOSED" => {
                    let msg = data["status"]["error"]["message"].as_str().unwrap_or("unknown");
                    return Err(DbError::Query(format!("Databricks statement failed: {msg}")));
                }
                _ => {
                    tokio::time::sleep(std::time::Duration::from_millis(500)).await;
                    let poll_resp = self.auth(self.client.get(&poll_url))
                        .send().await
                        .map_err(|e| DbError::Connection(e.to_string()))?;
                    data = poll_resp.json().await
                        .map_err(|e| DbError::Serialization(e.to_string()))?;
                }
            }
        }
        ...
        match (schema, rows) {
            (Some(cols), Some(rows)) => {
                rows.iter().map(|row| {
                    ...
                    cols.iter().zip(cells.iter()).map(|(col, cell)| {
                        ...
                        let val = match (type_name, cell) {
                            ...
                            ("LONG" | "INT" | "SHORT" | "BYTE", v) => {
                                v.as_str().and_then(|s| s.parse::<i64>().ok())
                                    .map(|n| Value::Number(n.into()))
                                    .unwrap_or(Value::Null)
                            }
                            ("DOUBLE" | "FLOAT" | "DECIMAL", v) => {
                                v.as_str().and_then(|s| s.parse::<f64>().ok())
                                    .and_then(|f| serde_json::Number::from_f64(f).map(Value::Number))
                                    .unwrap_or(Value::Null)
                            }
                            ("BOOLEAN", v) => Value::Bool(v.as_str() == Some("true")),
                            (_, v) => v.as_str().map(|s| Value::String(s.to_string())).unwrap_or(Value::Null),
                        };
                        (name, val)
                    }).collect()
                }).collect()
