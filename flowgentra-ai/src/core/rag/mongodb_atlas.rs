...
impl MongoAtlasVectorStore {
    ...
    fn filter_to_bson(f: &FilterExpr) -> mongodb::bson::Document {
        match f {
            FilterExpr::Eq(k, v) => {
                doc! { format!("metadata.{}", k): { "$eq":  bson_from_json(v) } }
            }
            FilterExpr::Ne(k, v) => {
                doc! { format!("metadata.{}", k): { "$ne":  bson_from_json(v) } }
            }
            FilterExpr::Gt(k, v) => {
                doc! { format!("metadata.{}", k): { "$gt":  bson_from_json(v) } }
            }
            FilterExpr::Lt(k, v) => {
                doc! { format!("metadata.{}", k): { "$lt":  bson_from_json(v) } }
            }
            FilterExpr::Gte(k, v) => {
                doc! { format!("metadata.{}", k): { "$gte": bson_from_json(v) } }
            }
            FilterExpr::Lte(k, v) => {
                doc! { format!("metadata.{}", k): { "$lte": bson_from_json(v) } }
            }
            FilterExpr::In(k, vs) => {
...
impl VectorStoreBackend for MongoAtlasVectorStore {
    ...
    async fn search(
        ...
        let bson_vec: Vec<Bson> = query_embedding
            .iter()
        ...
        if let Some(f) = filter {
            vs_stage.insert(
                "filter",
                Bson::Document(MongoAtlasVectorStore::filter_to_bson(&f)),
            );
        }
...
fn bson_from_json(v: &Value) -> Bson {
    match v {
        ...
        Value::Object(map) => {
            let doc: BsonDoc = map
                .iter()
                .map(|(k, v)| (k.clone(), bson_from_json(v)))
                .collect();
            Bson::Document(doc)
        }
    }
