    ...
    fn filter_to_os(f: &FilterExpr) -> Value {
        ...
        match f {
            ...
            FilterExpr::And(exprs) => {
                json!({ "bool": { "must":   exprs.iter().map(Self::filter_to_os).collect::<Vec<_>>() } })
            }
            FilterExpr::Or(exprs) => {
                json!({ "bool": { "should": exprs.iter().map(Self::filter_to_os).collect::<Vec<_>>() } })
            }
        }
    }
}
