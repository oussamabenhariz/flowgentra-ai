...
use crate::core::state::{State, StateUpdate};


/// Merge multiple StateUpdates into a new state using reducers
/// Deterministically merge multiple StateUpdate<T> into a new state using field-wise reducers
///
/// This function is generated per user state struct by the macro for full type safety and performance.
///
/// Usage in node execution:
///
/// 1. Each node returns a StateUpdate__YourState struct (with only changed fields set).
/// 2. Collect all StateUpdate__YourState from parallel nodes.
/// 3. Call merge_state(&current_state, updates, &Reducers__YourState::new()) to get the new state.
///
/// The macro generates the correct merge logic for each field and reducer.
pub fn merge_state<T: State>(
    _current: &T,
    _updates: Vec<StateUpdate<T>>,
    _reducers: &Reducers<T>,
) -> T {
    // This function is generated per user state struct by macro for full type safety and performance.
    // Pseudocode for generated logic:
    //
    // let mut new_state = current.clone();
    // for each field in T:
    //   let mut field_updates = updates.iter().filter_map(|u| u.field.clone()).collect::<Vec<_>>();
    //   field_updates.sort_by(...); // deterministic order (e.g., node id or insertion order)
    //   for update in field_updates {
    //     (reducers.field)(&mut new_state.field, update);
    //   }
    // return new_state
    unimplemented!("merge_state is generated by macro for each State type");
}

/// Reducers per field (generated by macro)
pub struct Reducers<T> {
    // Placeholder for macro-generated reducer set
