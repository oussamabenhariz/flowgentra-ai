...
pub fn create_planner_handler(
    ...
    Box::new(move |state: DynState| {
        ...
        Box::pin(async move {
            ...
            // Build a clean state view for the LLM:
            // - strip internal `_` fields (routing metadata, not pipeline data)
            // - keep all user fields, showing absent ones explicitly as null
            let state_json = state.to_json_string().unwrap_or_else(|_| "{}".to_string());
            let clean_state: serde_json::Value = serde_json::from_str(&state_json)
            ...
            let state_summary = truncate_for_prompt(
                &serde_json::to_string_pretty(&clean_state).unwrap_or_else(|_| "{}".to_string()),
                1500,
            );

            // List all non-END reachable nodes so the LLM knows what steps are available
            let available_steps: Vec<&str> = reachable
                .iter()
            ...
            let user_prompt = format!(
                "Available pipeline steps: {}\n\nCurrent pipeline state:\n{}\n\nBased on the state above, which node should run next?\nChoose from: {}\n(Reply with ONLY the node name, nothing else.)",
                available_steps.join(", "), state_summary, nodes_list
            );
