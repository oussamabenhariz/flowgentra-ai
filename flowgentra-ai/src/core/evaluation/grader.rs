...
impl LLMGrader {
    ...
    fn build_grading_prompt(output: &Value, task: &str, context: &str) -> String {
        let output_str = match serde_json::to_string_pretty(output) {
            Ok(s) => s,
...
"#,
    ...
    fn build_correction_grading_prompt(
        ...
        let original_str = match serde_json::to_string_pretty(original) {
            Ok(s) => s,
            Err(_) => original.to_string(),
        };
        let corrected_str = match serde_json::to_string_pretty(corrected) {
            Ok(s) => s,
            Err(_) => corrected.to_string(),
        };
...
"#,
    ...
    fn parse_json_grade(json: &Value) -> Result<GradeResult> {
        let obj = json.as_object().ok_or_else(|| {
            crate::core::error::FlowgentraError::LLMError("Invalid grading format".into())
        })?;
    ...
    pub fn generate_correction_prompt<T: State>(
        _output: &Value,
        grade: &GradeResult,
        _state: &T,
    ) -> String {
